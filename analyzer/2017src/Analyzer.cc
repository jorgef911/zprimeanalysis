...
void Analyzer::getGoodRecoJets(CUTS ePos, const PartStats& stats, const int syst) {
  ...
  for(auto lvec: *_Jet) {
    ...
    /// BJet specific
      ...
      else if(cut == "ApplyTightID") passCuts = passCuts && _Jet->passedTightJetID(i); //07.08.19
...
double Analyzer::getBJetSF(CUTS ePos, const PartStats& stats) {
  ...
  for( auto cut: stats.bset) {
	...
	if(oneb){
	  ...
	  //bjetSF1 = 0.561694 * ( (1. + (0.31439 * pt1) ) / (1. + (0.17756 * pt1) ) );
	  //////////////////////////////2016| pfCombinedInclusiveSecondaryVertexV2BJetTags//////////////////////////////
    //oneb:
    //'Loose' = 0.5426
    //bjetSF1 = 0.887973 * ( (1. + (0.0523821 * pt1) ) / (1. + (0.0460876 * pt1) ) );
    //'Medium' = 0.8484
    //bjetSF1 = 0.561694 * ( (1. + (0.31439 * pt1) ) / (1. + (0.17756 * pt1) ) );
    //'Tight' = 0.9535
    //bjetSF1 = 0.817647 * ( (1. + (0.038703* pt1)) /(1. + (0.0312388 * pt1) ) );

    //////////////////////////////2017| DeepFlavour=DeepJet//////////////////////////////
    //'Loose' = 0.0521
    //bjetSF1 = 1.04891*((1.+(0.0145976*pt1))/(1.+(0.0165274*pt1)));
    //'Medium' = 0.3033
    bjetSF1 = 0.991757*((1.+(0.0209615*pt1))/(1.+(0.0234962*pt1)));
    //'Tight' = 0.7489
    //bjetSF1 = 0.908648*((1.+(0.00516407*pt1))/(1.+(0.00564675*pt1)));

    //////////////////////////////2018| DeepJet=DeepFlavour//////////////////////////////
    //'Loose' = 0.0494
    //bjetSF1 = 0.873139+(0.00420739*(log(pt1+19)*(log(pt1+18)*(3-(0.380932*log(pt1+18))))));
    //'Medium' = 0.2770
    //bjetSF1 = 1.0097+(-(2.89663e-06*(log(pt1+19)*(log(pt1+18)*(3-(-(110.381*log(pt1+18))))))));
    //'Tight' = 0.7264
    //bjetSF1 = 0.818896+(0.00682971*(log(pt1+19)*(log(pt1+18)*(3-(0.440998*log(pt1+18))))));
	  }
	if(twob){
	  double pt1 = (_Jet->p4(active_part->at(CUTS::eRBJet)->at(0))).Pt();
    ...
    //'Medium'
    //bjetSF1 = 0.561694 * ( ( 1.+ (0.31439 *pt1) ) / (1. + (0.17756 * pt1) ) );
    //bjetSF2 = 0.561694 * ( ( 1.+ (0.31439 *pt2) ) / (1. + (0.17756 * pt2) ) );
    //'Tight'
    //bjetSF1 = 0.817647 * ( (1. + (0.038703* pt1)) /(1. + (0.0312388 * pt1) ) );
    //bjetSF2 = 0.817647 * ( (1. + (0.038703* pt2)) /(1. + (0.0312388 * pt2) ) );


    //////////////////////////////2017| DeepFlavour=DeepJet//////////////////////////////
    //'Loose'
    //bjetSF1 = 1.04891*((1.+(0.0145976*pt1))/(1.+(0.0165274*pt1)));
    //bjetSF2 = 1.04891*((1.+(0.0145976*pt2))/(1.+(0.0165274*pt2)));
    //'Medium'
    bjetSF1 = 0.991757*((1.+(0.0209615*pt1))/(1.+(0.0234962*pt1)));
    bjetSF2 = 0.991757*((1.+(0.0209615*pt2))/(1.+(0.0234962*pt2)));
    //'Tight'
    //bjetSF1 = 0.908648*((1.+(0.00516407*pt1))/(1.+(0.00564675*pt1)));
    //bjetSF2 = 0.908648*((1.+(0.00516407*pt2))/(1.+(0.00564675*pt2)));

    //////////////////////////////2018| DeepJet=DeepFlavour//////////////////////////////
    //'Loose'
   // bjetSF1 = 0.873139+(0.00420739*(log(pt1+19)*(log(pt1+18)*(3-(0.380932*log(pt1+18))))));
    //bjetSF2 = 0.873139+(0.00420739*(log(pt2+19)*(log(pt2+18)*(3-(0.380932*log(pt2+18))))));
    //'Medium'
    //bjetSF1 = 1.0097+(-(2.89663e-06*(log(pt1+19)*(log(pt1+18)*(3-(-(110.381*log(pt1+18))))))));
    //bjetSF2 = 1.0097+(-(2.89663e-06*(log(pt2+19)*(log(pt2+18)*(3-(-(110.381*log(pt2+18))))))));
    //'Tight'
    //bjetSF1 = 0.818896+(0.00682971*(log(pt1+19)*(log(pt1+18)*(3-(0.440998*log(pt1+18))))));
    //bjetSF2 = 0.818896+(0.00682971*(log(pt2+19)*(log(pt2+18)*(3-(0.440998*log(pt2+18))))));
	}
      }
  }
  ...
  for(auto lvec: *_FatJet) {
    ...
    for( auto cut: stats.bset) {
      if(!passCuts) break;
                                                     ...
                                                     passCutRange(_FatJet->SoftDropMass[i], stats.pmap.at("JetWmassCut")));
    // ----anti-overlap requirements
      else if(cut == "RemoveOverlapWithMuon1s") passCuts = passCuts && !isOverlaping(lvec, *_Muon, CUTS::eRMuon1, stats.dmap.at("Muon1MatchingDeltaR"));
      else if (cut =="RemoveOverlapWithMuon2s") passCuts = passCuts && !isOverlaping(lvec, *_Muon, CUTS::eRMuon2, stats.dmap.at("Muon2MatchingDeltaR"));
      else if(cut == "RemoveOverlapWithElectron1s") passCuts = passCuts && !isOverlaping(lvec, *_Electron, CUTS::eRElec1, stats.dmap.at("Electron1MatchingDeltaR"));
  ...
  for(auto i1 : *active_part->at(ePos1)) {
    ...
    for(auto i2 : *active_part->at(ePos2)) {
      ...
      for(auto cut : stats.bset) {
        ...
        else if (cut == "DiscrByLeadingLepton") {
          bool passCosDphiLL;
          bool passmTLL;
          if(part1.Pt()>part2.Pt()){
            double   CosDPhiLL = cos(absnormPhi(part1.Phi() - _MET->phi()));
            double   mTLL = calculateLeptonMetMt(part1);
            passCosDphiLL = passCutRange(CosDPhiLL, stats.pmap.at("CosDphiLeadingLeptonCut"));
            passmTLL = passCutRange(mTLL, stats.pmap.at("mTLeadingLeptonCut"));
          }
          else if(part1.Pt()<part2.Pt()){
            double   CosDPhiLL = cos(absnormPhi(part2.Phi() - _MET->phi()));
            double   mTLL = calculateLeptonMetMt(part2);
            passCosDphiLL = passCutRange(CosDPhiLL, stats.pmap.at("CosDphiLeadingLeptonCut"));
            passmTLL = passCutRange(mTLL, stats.pmap.at("mTLeadingLeptonCut"));
          }
          passCuts = passCuts && passCosDphiLL && passmTLL;
        }
        	...
        	double mass3= 0.0;
 /*	double pt3  = _MET->pt();
        double eta3 = _MET->eta();
        double phi3 = _MET->phi();
        double mass3= 0.0;*/
      	  ...
      	  double mrecpt= sqrt( pow(E1+E2+E3,2)
                            -pow(px1+px2+px3,2)
                            -pow(py1+py2+py3,2)
                            -pow(pz1+pz2+pz3,2) );
          passCuts = passCuts && passCutRange(mrecpt, stats.pmap.at("MassCut"));
        }
...
std::multimap<int,int> Analyzer::readinJSON(){ //NEW:  function for reading in the JSON file in the format that I established.  This happens once before events start entering preprocess.
  std::fstream fs(PUSPACE+"json2017.txt", std::fstream::in); //NEW:  takes the json file from where I have it (in the Pileup folder) and establishes that we'll take its input.
...
double Analyzer::getPreFireWeight(){
	double fireweigth=1.;
	if(active_part->at(CUTS::eDiJet)->size() >=1){
		double fire = 0;
		int DiJetsize = active_part->at(CUTS::eDiJet)->size();
		for(auto it : *active_part->at(CUTS::eDiJet)) {
			...
			if(DiJet.M() > fire){fire = DiJet.M();}
		}
		if(fire > 500 && fire <= 750) {fireweigth = 0.955;}
		else if (fire > 750 && fire <= 1000) {fireweigth = 0.945;}
		else if (fire > 1000 && fire <= 1250) {fireweigth = 0.911;}
		else if (fire > 1250 && fire <= 1500) {fireweigth = 0.8985;}
		else if (fire > 1500) {fireweigth = 0.867;}
		else {fireweigth = 1;}
	}
	return fireweigth;
}
...
void Analyzer::fill_histogram() {
  ...
  if(!isData){
    ...
    if(distats["Run"].bfind("ApplyPreFiring")){
	    wgt *= getPreFireWeight();
    }
...
//////////////////////////////////////////////////////////////
      ...
      histAddVal(cos(absnormPhi(part2.Phi() - part1.Phi())), 	"CosDphi");
      histAddVal ( cos(absnormPhi(llep.Phi()-_MET->phi())),     "leadinglcos1D");
      histAddVal ( calculateLeptonMetMt(llep),                	"leadinglmt1D");
